...
struct tweet_container {
  char tweet_name_array[20000][16];
};
...
struct tweeter_stat_container {
 struct tweeter_stat* individual_tweeter_stats[20000];
};
...
CsvParser *CsvParser_new(const char *filePath, const char *delimiter, int firstLineIsHeader) {
    ...
    if (filePath == ((void *)0)) {
        csvParser->filePath_ = ((void *)0);
    } else {
        int filePathLen = strlen(filePath);
        csvParser->filePath_ = (char*)malloc((filePathLen + 1));
        __builtin___strcpy_chk (csvParser->filePath_, filePath, __builtin_object_size (csvParser->filePath_, 2 > 1 ? 1 : 0));
    }
    csvParser->firstLineIsHeader_ = firstLineIsHeader;
    csvParser->errMsg_ = ((void *)0);
    if (delimiter == ((void *)0)) {
    ...
    csvParser->header_ = ((void *)0);
    csvParser->fileHandler_ = ((void *)0);
 csvParser->fromString_ = 0;
 csvParser->csvString_ = ((void *)0);
...
CsvParser *CsvParser_new_from_string(const char *csvString, const char *delimiter, int firstLineIsHeader) {
 CsvParser *csvParser = CsvParser_new(((void *)0), delimiter, firstLineIsHeader);
 csvParser->fromString_ = 1;
 if (csvString != ((void *)0)) {
  int csvStringLen = strlen(csvString);
  csvParser->csvString_ = (char*)malloc(csvStringLen + 1);
  __builtin___strcpy_chk (csvParser->csvString_, csvString, __builtin_object_size (csvParser->csvString_, 2 > 1 ? 1 : 0));
 }
 return csvParser;
}
...
void CsvParser_destroy(CsvParser *csvParser) {
    ...
    if (csvParser->filePath_ != ((void *)0)) {
        free(csvParser->filePath_);
    }
    if (csvParser->errMsg_ != ((void *)0)) {
        free(csvParser->errMsg_);
    }
    if (csvParser->fileHandler_ != ((void *)0)) {
        fclose(csvParser->fileHandler_);
    }
    if (csvParser->header_ != ((void *)0)) {
        CsvParser_destroy_row(csvParser->header_);
    }
 if (csvParser->csvString_ != ((void *)0)) {
...
void _CsvParser_setErrorMessage(CsvParser *csvParser, const char *errorMessage) {
    ...
    __builtin___strcpy_chk (csvParser->errMsg_, errorMessage, __builtin_object_size (csvParser->errMsg_, 2 > 1 ? 1 : 0));
}
...
CsvRow *_CsvParser_getRow(CsvParser *csvParser) {
    ...
    if (csvParser->filePath_ == ((void *)0) && (! csvParser->fromString_)) {
        _CsvParser_setErrorMessage(csvParser, "Supplied CSV file path is NULL");

        return ((void *)0);
    }
    if (csvParser->csvString_ == ((void *)0) && csvParser->fromString_) {
    ...
    if (! csvParser->fromString_) {
        if (csvParser->fileHandler_ == ((void *)0)) {
            ...
            if (csvParser->fileHandler_ == ((void *)0)) {
                int errorNum = (*__error());
                const char *errStr = strerror(errorNum);
                char *errMsg = (char*)malloc(1024 + strlen(errStr));
                __builtin___strcpy_chk (errMsg, "", __builtin_object_size (errMsg, 2 > 1 ? 1 : 0));
                __builtin___sprintf_chk (errMsg, 0, __builtin_object_size (errMsg, 2 > 1 ? 1 : 0), "Error opening CSV file for reading: %s : %s", csvParser->filePath_, errStr);
    ...
    while (1) {
        ...
        if (isEndOfFile || ((currChar == csvParser->delimiter_ || currChar == '\n') && ! inside_complex_field) ){
            ...
            __builtin___strcpy_chk (csvRow->fields_[fieldIter], currField, __builtin_object_size (csvRow->fields_[fieldIter], 2 > 1 ? 1 : 0));
            free(currField);
            csvRow->numOfFields_++;
            if (currChar == '\n') {
                return csvRow;
...
CsvRow *CsvParser_getRow(CsvParser *csvParser) {
    if (csvParser->firstLineIsHeader_ && csvParser->header_ == ((void *)0)) {
...
int getNumOcs(char* str, char* needle) {
    ...
    __builtin___strncpy_chk (p, str, 1000, __builtin_object_size (p, 2 > 1 ? 1 : 0));
    int total = 0;
    while ( (p=strstr(p, needle)) != ((void *)0) )
    {
        total++;
        p++;
    }
...
int main(int argc, char **args) {
    ...
    if (fgets (header_string, 2000, fp) == ((void *)0)) {
        exit(0);
    }

    char header_copy[2000];
    __builtin___strncpy_chk (header_copy, header_string, 1000, __builtin_object_size (header_copy, 2 > 1 ? 1 : 0));
    ...
    while ((row = CsvParser_getRow(csvparser)) ) {
        ...
        printf("FIELD: %s\n", rowFields[name_spot]);
        __builtin___strncpy_chk (all_tweeters->tweet_name_array[tweet_count], rowFields[name_spot], 16, __builtin_object_size (all_tweeters->tweet_name_array[tweet_count], 2 > 1 ? 1 : 0));
    ...
    for (int i = 0; i < n - 1 ; i++) {
     for (int j = i + 1; j < n; j++) {
      ...
      if (strcmp(all_tweeters->tweet_name_array[i], all_tweeters->tweet_name_array[j]) > 0)
            {
             __builtin___strncpy_chk (temp, all_tweeters->tweet_name_array[i], 16, __builtin_object_size (temp, 2 > 1 ? 1 : 0));
                __builtin___strncpy_chk (all_tweeters->tweet_name_array[i], all_tweeters->tweet_name_array[j], 16, __builtin_object_size (all_tweeters->tweet_name_array[i], 2 > 1 ? 1 : 0));
                __builtin___strncpy_chk (all_tweeters->tweet_name_array[j], temp, 16, __builtin_object_size (all_tweeters->tweet_name_array[j], 2 > 1 ? 1 : 0));
            }
    ...
    stats = malloc(sizeof(struct tweeter_stat) * 20000);

    for (int i = 0; i < tweet_count - 1; i++) {
      count = 0;

      __builtin___strncpy_chk (stats[unique_members].name, all_tweeters->tweet_name_array[i], 16, __builtin_object_size (stats[unique_members].name, 2 > 1 ? 1 : 0));
